...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  ...
  // Teste que a função retorna, num objeto, os dados corretos de um círculo de raio 3.
    ...
    expect(parseFloat((circle(2).circumference).toPrecision(4))).toBe(12.56);
    expect(parseFloat((circle(3).area).toPrecision(4))).toBe(28.26);
    expect(circle(3)).toEqual({radius: 3, area: (3.14 * 3 * 3), circumference: 18.84}); // Construída com ajuda na Monitoria Summer do Douglas e com a ajuda do José Carlos da Turma 19B 
  });
});
